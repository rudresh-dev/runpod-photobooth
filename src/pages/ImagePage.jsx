...
const ImagePage = () => {
  ...
  return (
    <div className="w-screen h-screen bg-red-700">
      ...
      {!isCaptured ? (
        <div className='w-screen h-screen'>
          <Webcam
            ...
            className="webcam"
          />
          <button className='z-30 absolute bottom-14 left-[50vw]' onClick={handleCapture}>
      ...
      {isCaptured && (
        <button className='absolute bottom-14 right-[50vw]' onClick={handleNext} disabled={isProcessing}>
          {isProcessing ? <SiNextra size={80} color='white' /> : <GrCaretNext size={80} color='white' />}
        </button>
      )}
